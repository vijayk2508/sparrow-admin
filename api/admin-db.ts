...
async function runSeed(force) {
  var counts = {}; var seeded = []; var skipped = [];
  ...
  var cols = [["coaches", SEED.coaches], ["programs", SEED.programs], ["schedule", SEED.schedule], ["membershipPlans", SEED.membershipPlans], ["testimonials", SEED.testimonials], ["gallery", SEED.gallery], ["events", SEED.events]];
  for (var ci = 0; ci < cols.length; ci++) {
    var path = cols[ci][0]; var docs = cols[ci][1];
    ...
    var rows = docs.map(function (d, i) { return toSeedRow(d, i, path); });
    var CH = 200;

...
export default async function handler(req: any, res: any) {
  ...
  if (req.method !== "GET" && req.method !== "PATCH") { res.status(405).json({ success: false, message: "Method not allowed" }); return; }
  ...
  try {
    var user = await syncUser(identity);
    if (req.method === "GET") { res.status(200).json({ success: true, data: user }); return; }
    var body = req.body || {};
    var profilePatch: Record<string, any> = { updated_at: new Date().toISOString() };
    if (typeof body.full_name === "string") profilePatch.full_name = body.full_name.slice(0, 200);
    if (typeof body.avatar_url === "string") profilePatch.avatar_url = body.avatar_url.slice(0, 1000);
    var updated = await sbRest("users?" + new URLSearchParams({ firebase_uid: "eq." + identity.uid }).toString(), { method: "PATCH", prefer: "return=representation", body: profilePatch });
    var updatedRow = (updated && updated[0]) || null;
    if (!updatedRow) { res.status(500).json({ success: false, message: "Profile update failed" }); return; }
    res.status(200).json({ success: true, data: { id: updatedRow.id, firebaseUid: updatedRow.firebase_uid, email: updatedRow.email, fullName: (updatedRow.full_name === undefined ? null : updatedRow.full_name), avatarUrl: (updatedRow.avatar_url === undefined ? null : updatedRow.avatar_url), role: (updatedRow.role === "admin" ? "admin" : "user") } });
  } catch (e: any) {
    res.status(500).json({ success: false, message: (e && e.message) || "Request failed" });
  }
}

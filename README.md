...
Other scripts: `npm run build` (production bundle in `dist/`), `npm run lint` (tsc), `npm run preview`.

## One-time Supabase setup

1. Create a project at https://supabase.com (if you don't have one).
2. SQL Editor → paste `../sparrow-training-club/supabase/schema.sql` → Run. This creates all tables, RLS policies, the `admins` table, the `media` storage bucket and realtime publications.
...
- Image uploads go to the public `media` bucket; only admins can write (storage RLS).
